...
def register(request):
	...
	if request.method == 'POST':
		...
		d1 = request.POST.get('degree')
		d2 = request.POST.get('depart')
		d3 = request.POST.get('major_skill')
		...
		if user_exist:
			...
			if phone == 10:
				...
				country=country,state=state,city=city,address=addr,degree_id=d1,depart_id=d2,skill=d3,resume=resume)
				if crt:
					messages.success(request,'Registered Successfully')
			else:
				messages.success(request,'Phone Number Must Contain 10 Digit')
	return render(request,'register.html',{'detail':detail,'dept':dept,'skill':skill})
...
def search_resume(request):
	if request.session.has_key('admin'):
		...
		if request.method == 'POST':
			...
			major_skill = request.POST.get('major_skill')
			if request.POST.get('depart') == '' and request.POST.get('major_skill') == '':
				cursor = connection.cursor()
				sql = '''SELECT u.name,u.email,u.phone_number,u.degree_id,u.depart_id,u.skill,u.resume 
				from app_user_detail as u where u.degree_id='%s' OR u.depart_id='%s' OR u.skill='%s' ''' % (degree,depart,major_skill)
				post = cursor.execute(sql)
				row = cursor.fetchall()
			elif request.POST.get('major_skill') == '':
				cursor = connection.cursor()
				sql = '''SELECT u.name,u.email,u.phone_number,u.degree_id,u.depart_id,u.skill,u.resume 
				from app_user_detail as u where u.degree_id='%s' AND u.depart_id='%s' OR u.skill='%s' ''' % (degree,depart,major_skill)
				post = cursor.execute(sql)
				row = cursor.fetchall()
			elif request.POST.get('depart') == '' and request.POST.get('degree') == '':
				cursor = connection.cursor()
				sql = '''SELECT u.name,u.email,u.phone_number,u.degree_id,u.depart_id,u.skill,u.resume 
				from app_user_detail as u where u.degree_id='%s' OR u.depart_id='%s' OR u.skill='%s' ''' % (degree,depart,major_skill)
				post = cursor.execute(sql)
				...
				cursor = connection.cursor()
				sql = '''SELECT u.name,u.email,u.phone_number,u.degree_id,u.depart_id,u.skill,u.resume 
				from app_user_detail as u where u.degree_id='%s' AND u.depart_id='%s' AND u.skill='%s' ''' % (degree,depart,major_skill)
				post = cursor.execute(sql)
				row = cursor.fetchall()
			return render(request,'search_resume.html',{'detail':detail,'dept':dept,'skill':skill,'row':row})
		return render(request,'search_resume.html',{'detail':detail,'dept':dept,'skill':skill})
	else:
		return render(request,'admin_login.html',{})